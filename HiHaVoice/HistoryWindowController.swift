...
class HistoryWindowController: NSObject, NSWindowDelegate {
    ...
    private let windowIdentifier = NSUserInterfaceItemIdentifier("be.hiha.voice.historyWindow")
    private let windowAutosaveName = NSWindow.FrameAutosaveName("HiHaVoiceHistoryWindowFrame")

    private override init() {
        super.init()
    }

    func showHistoryWindow(modelContainer: ModelContainer, engine: HiHaVoiceEngine) {
    ...
    private func createHistoryWindow(modelContainer: ModelContainer, engine: HiHaVoiceEngine) -> NSWindow {
        ...
        window.title = "Hi-Ha Voice — Transcription History"
