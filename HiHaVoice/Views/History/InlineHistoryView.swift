...
struct InlineHistoryView: View {
    ...
    private let exportService = HiHaVoiceCSVExportService()
    ...
    var body: some View {
        ...
        .animation(.smooth(duration: 0.3), value: isPanelPresented)
        .alert("Supprimer les éléments sélectionnés ?", isPresented: $showDeleteConfirmation) {
            Button("Supprimer", role: .destructive) {
                deleteSelectedTranscriptions()
            }
            Button("Annuler", role: .cancel) {}
        } message: {
            Text("Cette action est irréversible. Supprimer \(selectedTranscriptions.count) élément\(selectedTranscriptions.count == 1 ? "" : "s") ?")
        }
        .onAppear {
        ...
        HStack(spacing: 10) {
            HStack(spacing: 6) {
                Image(systemName: "magnifyingglass")
                    ...
                    .font(.system(size: 12))
                TextField("Rechercher des transcriptions…", text: $searchText)
    ...
    private var selectionBar: some View {
        HStack(spacing: 16) {
            Text("\(selectedTranscriptions.count) sélectionné(s)")
            ...
            Button(action: {
                ...
                Label("Analyser", systemImage: "chart.bar.xaxis")
    ...
    private var emptyStateView: some View {
        VStack(spacing: 12) {
            ...
            Image(systemName: "doc.text.magnifyingglass")
                .font(.system(size: 40))
                .foregroundColor(.secondary)
            Text(searchText.isEmpty ? "Aucune transcription pour l'instant" : "Aucun résultat")
                .font(.system(size: 16, weight: .medium))
                .foregroundColor(.secondary)
            Text(searchText.isEmpty ? "Ton historique de transcriptions apparaîtra ici" : "Essaie un autre terme de recherche")
                .font(.system(size: 13))
                ...
                Section {
                    Button(action: {
                        ...
                        HStack(spacing: 8) {
                            ...
                            Text(isLoading ? "Chargement…" : "Charger plus")
                                .font(.system(size: 13, weight: .medium))
                        }
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 4)
