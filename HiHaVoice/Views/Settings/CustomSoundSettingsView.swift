...
struct CustomSoundSettingsView: View {
    ...
    var body: some View {
        Group {
            LabeledContent("Son de démarrage") {
                soundControls(for: .start)
            }

            LabeledContent("Son d'arrêt") {
    ...
    private func soundControls(for type: CustomSoundManager.SoundType) -> some View {
        ...
        HStack(spacing: 8) {
            Text(isCustom ? (fileName ?? "Personnalisé") : "Par défaut")
    ...
    private func selectSound(for type: CustomSoundManager.SoundType) {
        ...
        panel.title = "Choisir le son \(type.rawValue.capitalized)"
        panel.message = "Sélectionne un fichier audio"
        ...
        panel.begin { response in
            ...
            if case .failure(let error) = result {
                alertTitle = "Fichier audio invalide"
