...
struct CloudModelCardView: View {
            ...
            if isConfigured {
                Menu {
                    Button {
                        ...
                        Label("Retirer la clé API", systemImage: "trash")
                    }
                } label: {
    ...
    private var configurationSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 8) {
                SecureField("Entre ta clé API \(model.provider.rawValue)", text: $apiKey)
                ...
                Button(action: verifyAPIKey) {
                    ...
                    .background(
                        Capsule()
                            .fill(verificationStatus == .success ? Color.HiHa.auroraCyan : Color.HiHa.sovereignMarine)
                    )
                }
                .buttonStyle(.plain)
            ...
            if verificationStatus == .failure {
                ...
                Text("Clé API vérifiée avec succès !")
                    .font(.caption)
    ...
    private func verifyAPIKey() {
        ...
        guard let cloudProvider = CloudProviderRegistry.provider(for: model.provider) else {
            ...
            verificationError = "Fournisseur non pris en charge"
            return
        }
