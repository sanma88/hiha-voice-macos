...
@main
struct HiHaVoiceApp: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) var appDelegate
    let container: ModelContainer
    let containerInitializationFailed: Bool

    @StateObject private var engine: HiHaVoiceEngine
    ...
    init() {
        ...
        let logger = Logger(subsystem: "be.hiha.voice", category: "Initialization")
        let schema = Schema([
            Transcription.self,
            VocabularyWord.self,
        ...
        // Attempt 2: Try in-memory storage
            ...
            DispatchQueue.main.async {
                ...
                alert.informativeText = "Hi-Ha Voice n'a pas pu accéder à son espace de stockage. Tes transcriptions ne seront pas sauvegardées entre les sessions."
        ...
        let appSupportDirectory = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("be.hiha.voice")
    ...
    private static func createPersistentContainer(schema: Schema, logger: Logger) -> ModelContainer? {
        do {
            ...
            let appSupportURL = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
                .appendingPathComponent("be.hiha.voice", isDirectory: true)
            ...
            #else
            let dictionaryCloudKit: ModelConfiguration.CloudKitDatabase = .private("iCloud.be.hiha.voice")
            #endif
        ...
        WindowGroup {
            if hasCompletedOnboarding {
                ContentView()
                    ...
                    .tint(Color.HiHa.auroraCyan)
                    .onAppear {
                        // Check if container initialization failed
                        if containerInitializationFailed {
                            let alert = NSAlert()
                            alert.messageText = "Erreur critique de stockage"
                            alert.informativeText = "Hi-Ha Voice ne peut pas initialiser son système de stockage. L'application ne peut pas continuer.\n\nRéinstalle l'app ou contacte le support si le problème persiste."
                        ...
                        if let pendingURL = appDelegate.pendingOpenFileURL {
                            NotificationCenter.default.post(name: .navigateToDestination, object: nil, userInfo: ["destination": "Transcrire un fichier"])
                ...
                OnboardingView(hasCompletedOnboarding: $hasCompletedOnboarding)
                    ...
                    .background(WindowAccessor { window in
                        if window.identifier == nil || window.identifier != NSUserInterfaceItemIdentifier("be.hiha.voice.onboardingWindow") {
...
struct CheckForUpdatesView: View {
    ...
    var body: some View {
        Button("Vérifier les mises à jour…", action: updaterViewModel.checkForUpdates)
