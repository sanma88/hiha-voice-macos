...
struct OnboardingPermissionsView: View {
    ...
    private let permissions: [OnboardingPermission] = [
        OnboardingPermission(
            ...
            description: "Active ton micro pour parler et convertir ta voix en texte instantanément.",
            icon: "waveform",
            type: .microphone
        ),
        OnboardingPermission(
            title: "Choix du micro",
            description: "Sélectionne le périphérique audio que tu veux utiliser avec Hi-Ha Voice.",
            icon: "headphones",
            type: .audioDeviceSelection
        ),
        OnboardingPermission(
            title: "Accès à l'Accessibilité",
            description: "Autorise Hi-Ha Voice à t'aider à écrire partout sur ton Mac.",
            icon: "accessibility",
            type: .accessibility
        ),
        OnboardingPermission(
            title: "Enregistrement de l'écran",
            description: "Cela permet d'améliorer la précision de la transcription.",
            icon: "rectangle.inset.filled.and.person.filled",
            type: .screenRecording
        ),
        OnboardingPermission(
            title: "Raccourci clavier",
            description: "Configure un raccourci clavier pour accéder rapidement à Hi-Ha Voice de n'importe où.",
            icon: "keyboard",
                    ...
                    VStack(spacing: 40) {
                        ...
                        HStack(spacing: 8) {
                            ForEach(0..<permissions.count, id: \.self) { index in
                                Circle()
                                    .fill(index <= currentPermissionIndex ? Color.HiHa.sovereignMarine : Color.white.opacity(0.1))
                            ...
                            VStack(spacing: 12) {
                                HStack(spacing: 8) {
                                    ...
                                    if permissions[currentPermissionIndex].type == .screenRecording {
                                        InfoTip(
                                            "Hi-Ha Voice capture le texte à l'écran pour comprendre le contexte de ta voix, ce qui améliore nettement la précision de la transcription. Ta vie privée compte : ces données sont traitées localement et ne sont pas stockées.",
                                ...
                                VStack(spacing: 20) {
                                    if audioDeviceManager.availableDevices.isEmpty {
                                        ...
                                        styledPicker(
                                            ...
                                            displayValue: audioDeviceManager.availableDevices.first { $0.id == audioDeviceManager.selectedDeviceID }?.name ?? "Choisir un périphérique",
                                            options: audioDeviceManager.availableDevices.map { $0.id },
                                            optionDisplayName: { deviceId in
                                                audioDeviceManager.availableDevices.first { $0.id == deviceId }?.name ?? "Périphérique inconnu"
                                            },
                                    ...
                                    Text("Pour de meilleurs résultats, on te recommande d'utiliser le micro intégré de ton Mac.")
                                        .font(.caption)
                                        .foregroundColor(.white.opacity(0.7))
                                        .multilineTextAlignment(.center)
                                        .padding(.horizontal)
                        ...
                        VStack(spacing: 16) {
                            ...
                            if !permissionStates[currentPermissionIndex] && 
                               ...
                               permissions[currentPermissionIndex].type != .audioDeviceSelection {
                                SkipButton(text: "Ignorer pour le moment") {
                                    moveToNext()
                                }
                            }
                        }
    ...
    private func getButtonTitle() -> String {
        switch permissions[currentPermissionIndex].type {
        case .keyboardShortcut:
            return permissionStates[currentPermissionIndex] ? "Continuer" : "Définir un raccourci"
        case .audioDeviceSelection:
            return "Continuer"
        default:
            return permissionStates[currentPermissionIndex] ? "Continuer" : "Activer l'accès"
        }
    }
