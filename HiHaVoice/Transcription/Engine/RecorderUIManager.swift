...
class RecorderUIManager: ObservableObject {
    ...
    private let logger = Logger(subsystem: "be.hiha.voice", category: "RecorderUIManager")

    init() {}

    /// Call after HiHaVoiceEngine is created to break the circular init dependency.
    func configure(engine: HiHaVoiceEngine, recorder: Recorder) {
        self.engine = engine
