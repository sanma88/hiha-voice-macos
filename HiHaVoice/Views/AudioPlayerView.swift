            ...
            ZStack(alignment: .leading) {
                if isLoading {
                    ...
                    if isHovering {
                        ...
                        Rectangle()
                            .fill(LinearGradient.hiHaBrandInk)
                            .frame(width: 2)
                            .frame(maxHeight: .infinity)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 8)
                .fill(isError ? Color.HiHa.signalCoral.opacity(0.1) : Color.HiHa.auroraCyan.opacity(0.1))
                .stroke(isError ? Color.HiHa.signalCoral.opacity(0.2) : Color.HiHa.auroraCyan.opacity(0.2), lineWidth: 1)
        )
...
struct AudioPlayerView: View {
        ...
        VStack(spacing: 8) {
            ...
            HStack(spacing: 8) {
                ...
                HStack(spacing: 8) {
                    ...
                    .help("Choisir un prompt d'amélioration IA")
                    ...
                    if transcription != nil {
                        ...
                        .help("Réappliquer l'amélioration IA avec le prompt sélectionné")
                    }
        ...
        .overlay(
            VStack {
                if let state = bannerState {
                    switch state {
                    case .retranscribeSuccess:
                        StatusBanner(message: "Retranscription réussie", isError: false)
                    case .reEnhanceSuccess:
                        StatusBanner(message: "Amélioration IA réappliquée", isError: false)
                    case .retranscribeError(let message):
                        StatusBanner(message: message.isEmpty ? "Échec de la retranscription" : message, isError: true)
                    case .reEnhanceError(let message):
                        StatusBanner(message: message.isEmpty ? "Échec de l'amélioration IA" : message, isError: true)
                    }
                }
                Spacer()
            }
    ...
    private func reEnhanceOnly() {
        ...
        guard enhancementService.isEnhancementEnabled, enhancementService.isConfigured else {
            showTemporaryBanner(.reEnhanceError("L'amélioration IA n'est pas activée ou configurée"))
            return
        }
    ...
    private func retranscribeAudio() {
        guard let currentTranscriptionModel = engine.transcriptionModelManager.currentTranscriptionModel else {
            showTemporaryBanner(.retranscribeError("Aucun modèle de transcription sélectionné"))
            return
        }
