...
struct HiHaVoiceExportedSettings: Codable {
    let version: String
...
class ImportExportService {
    ...
    func exportSettings(enhancementService: AIEnhancementService, whisperPrompt: WhisperPrompt, hotkeyManager: HotkeyManager, menuBarManager: MenuBarManager, mediaController: MediaController, playbackController: PlaybackController, soundManager: SoundManager, recorderUIManager: RecorderUIManager, modelContext: ModelContext) {
        ...
        let exportedSettings = HiHaVoiceExportedSettings(
        ...
        do {
            ...
            savePanel.nameFieldStringValue = "HiHaVoice_Settings_Backup.json"
            savePanel.title = "Export Hi-Ha Voice Settings"
                ...
                if savePanel.runModal() == .OK {
                    if let url = savePanel.url {
                        do {
                            ...
                            self.showAlert(title: "Export réussi", message: "Tes réglages ont été exportés vers \(url.lastPathComponent).")
                        } catch {
                            self.showAlert(title: "Erreur d'export", message: "Impossible d'enregistrer les réglages : \(error.localizedDescription)")
                        }
                    }
                } else {
                    self.showAlert(title: "Export annulé", message: "L'opération d'export a été annulée.")
                }
            }
        } catch {
            self.showAlert(title: "Erreur d'export", message: "Impossible d'encoder les réglages : \(error.localizedDescription)")
        }
    }
    ...
    func importSettings(enhancementService: AIEnhancementService, whisperPrompt: WhisperPrompt, hotkeyManager: HotkeyManager, menuBarManager: MenuBarManager, mediaController: MediaController, playbackController: PlaybackController, soundManager: SoundManager, recorderUIManager: RecorderUIManager, modelContext: ModelContext, transcriptionModelManager: TranscriptionModelManager) {
        ...
        openPanel.title = "Import Hi-Ha Voice Settings"
        ...
        DispatchQueue.main.async {
            if openPanel.runModal() == .OK {
                guard let url = openPanel.url else {
                    self.showAlert(title: "Erreur d'import", message: "Impossible de récupérer l'URL du fichier.")
                    return
                }
                ...
                do {
                    ...
                    let importedSettings = try decoder.decode(HiHaVoiceExportedSettings.self, from: jsonData)
                    ...
                    self.showAlert(title: "Erreur d'import", message: "Erreur lors de l'import : \(error.localizedDescription). Le fichier est peut-être corrompu ou dans un format incorrect.")
                }
            } else {
                self.showAlert(title: "Import annulé", message: "L'opération d'import a été annulée.")
            }
        }
    }
        ...
        DispatchQueue.main.async {
            ...
            if response == .alertSecondButtonReturn {
                NotificationCenter.default.post(
                    ...
                    object: nil,
                    userInfo: ["destination": "Amélioration IA"]
                )
            }
        }
    }
