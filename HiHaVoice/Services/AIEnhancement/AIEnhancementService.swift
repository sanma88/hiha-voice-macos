...
class AIEnhancementService: ObservableObject {
    private let logger = Logger(subsystem: "be.hiha.voice", category: "AIEnhancementService")
        ...
        if aiService.selectedProvider == .ollama {
            do {
                ...
                if let localError = error as? LocalAIError {
                    throw EnhancementError.customError(localError.errorDescription ?? "Une erreur Ollama inconnue est survenue.")
                } else {
        ...
        if aiService.selectedProvider == .localCLI {
            do {
                ...
                if let localError = error as? LocalCLIError {
                    throw EnhancementError.customError(localError.errorDescription ?? "Une erreur Local CLI inconnue est survenue.")
                } else {
                    throw EnhancementError.customError(error.localizedDescription)
                }
            }
        ...
        do {
            ...
            default:
                guard let baseURL = URL(string: aiService.selectedProvider.baseURL) else {
                    throw EnhancementError.customError("\(aiService.selectedProvider.rawValue) a une URL de point d'API invalide. Mets-la à jour dans les réglages IA.")
                }
    ...
    private func mapLLMKitError(_ error: LLMKitError) -> EnhancementError {
        ...
        case .invalidURL, .decodingError, .encodingError:
            return .customError(error.localizedDescription ?? "Une erreur inconnue est survenue.")
        }
    }
...
extension EnhancementError: LocalizedError {
    var errorDescription: String? {
        switch self {
        case .notConfigured:
            return "Fournisseur IA non configuré. Vérifie ta clé API."
        case .invalidResponse:
            return "Réponse invalide du fournisseur IA."
        case .enhancementFailed:
            return "L'amélioration IA n'a pas pu traiter le texte."
        case .networkError:
            return "Connexion réseau échouée. Vérifie ton accès à Internet."
        case .serverError:
            return "Le serveur du fournisseur IA a rencontré une erreur. Réessaye plus tard."
        case .rateLimitExceeded:
            return "Limite de requêtes atteinte. Réessaye plus tard."
        case .timeout:
            return "Délai d'amélioration dépassé. Vérifie ta connexion ou augmente le délai d'attente."
