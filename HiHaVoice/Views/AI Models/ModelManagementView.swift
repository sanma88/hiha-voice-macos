...
struct ModelManagementView: View {
    ...
    var body: some View {
        ...
        .alert(isPresented: $isShowingDeleteAlert) {
            Alert(
                ...
                primaryButton: .destructive(Text("Supprimer"), action: deleteActionClosure),
    ...
    private var defaultModelSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Modèle par défaut")
                .font(.headline)
                .foregroundColor(.secondary)
            Text(transcriptionModelManager.currentTranscriptionModel?.displayName ?? "Aucun modèle sélectionné")
                .font(.title2)
                .fontWeight(.bold)
        }
        .padding()
            ...
            HStack {
                ...
                Button(action: {
                    ...
                    Image(systemName: "gear")
                        ...
                        .foregroundColor(isShowingSettings ? Color.HiHa.sovereignMarine : .primary.opacity(0.7))
                        .padding(12)
                        .background(
            ...
            VStack(spacing: 12) {
                        ...
                        ModelCardView(
                            ...
                            deleteAction: {
                                if let customModel = model as? CustomCloudModel {
                                    alertTitle = "Supprimer le modèle personnalisé"
                                    alertMessage = "Es-tu sûr·e de vouloir supprimer le modèle personnalisé « \(customModel.displayName) » ?"
                                    deleteActionClosure = {
                                    ...
                                    alertTitle = "Supprimer le modèle"
                                    alertMessage = "Es-tu sûr·e de vouloir supprimer le modèle « \(downloadedModel.name) » ?"
                                    deleteActionClosure = {
                                        Task {
                    ...
                    if selectedFilter == .local {
                        HStack(spacing: 8) {
                            ...
                            InfoTip(
                                "Ajoute un modèle whisper personnalisé (fine-tuné) pour l'utiliser avec Hi-Ha Voice. Sélectionne le fichier .bin téléchargé.",
                                learnMoreURL: "https://hi-ha.be"
                            )
                            .help("En savoir plus sur les modèles locaux personnalisés")
                        }
                    }
    ...
    private var intelMacWarningBanner: some View {
        HStack(spacing: 10) {
            ...
            Text("Les modèles locaux ne sont pas fiables sur Mac Intel")
            ...
            Button(action: {
                ...
                .padding(.vertical, 6)
                .background(Color.HiHa.electricViolet.opacity(0.12))
                .cornerRadius(6)
            }
            .buttonStyle(.plain)
        }
        ...
        .padding(.vertical, 10)
        .background(Color.HiHa.electricViolet.opacity(0.08))
        .cornerRadius(8)
    }
    ...
    private func presentImportPanel() {
        ...
        panel.title = "Choisir un modèle Whisper ggml .bin"
