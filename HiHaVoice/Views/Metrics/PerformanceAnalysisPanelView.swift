...
struct PerformanceAnalysisPanelView: View {
    ...
    private var summarySection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 10) {
                summaryPill(icon: "doc.text.fill", value: "\(analysis.totalTranscripts)", label: "Total", color: Color.HiHa.electricViolet)
                summaryPill(icon: "waveform.path.ecg", value: "\(analysis.totalWithTranscriptionData)", label: "Analysables", color: Color.HiHa.auroraCyan)
                summaryPill(icon: "sparkles", value: "\(analysis.totalEnhancedFiles)", label: "Améliorées", color: Color.HiHa.auroraCyan)
            }
        }
    }
    ...
    private var systemInfoSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            sectionHeader("Informations système")

            VStack(spacing: 0) {
                infoRow(label: "Appareil", value: PerformanceAnalyzer.getMacModel())
                Divider().padding(.horizontal, 10)
                infoRow(label: "Processeur", value: PerformanceAnalyzer.getCPUInfo())
                Divider().padding(.horizontal, 10)
                infoRow(label: "Mémoire", value: PerformanceAnalyzer.getMemoryInfo())
            }
            .background(
    ...
    private var transcriptionModelsSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            sectionHeader("Modèles de transcription")
    ...
    private func transcriptionModelTile(_ modelStat: PerformanceAnalyzer.ModelStat) -> some View {
        VStack(spacing: 10) {
            ...
            VStack(spacing: 2) {
                ...
                Text("\(modelStat.fileCount) transcriptions")
                    .font(.system(size: 10))
                    .foregroundColor(.secondary)
            }
            .frame(maxWidth: .infinity)
            ...
            VStack(spacing: 3) {
                ...
                Text("Plus rapide que le temps réel")
                    .font(.system(size: 10))
                    .foregroundColor(.secondary)
            }
        ...
        .padding(14)
        .background(MetricCardBackground(color: Color.HiHa.auroraCyan))
        .cornerRadius(12)
    }
    ...
    private var enhancementModelsSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            sectionHeader("Modèles d'Amélioration IA")
        ...
        VStack(spacing: 10) {
            ...
            VStack(spacing: 3) {
                Text(String(format: "%.2f s", modelStat.avgProcessingTime))
                    .font(.system(size: 24, weight: .bold, design: .rounded))
                    .foregroundColor(Color.HiHa.electricViolet)
                Text("Temps d'amélioration moyen")
                    .font(.system(size: 10))
                    .foregroundColor(.secondary)
            }
        }
        .padding(14)
        .background(MetricCardBackground(color: Color.HiHa.electricViolet))
        .cornerRadius(12)
    }
